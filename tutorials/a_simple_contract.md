...
The contract API also provides a system contract accessible via the running chaincode with the name `org.hyperledger.fabric`. This system contract currently provides a single function `GetMetadata` which provides details on the contracts running within the chaincode. Query this function to see details about your contract:
